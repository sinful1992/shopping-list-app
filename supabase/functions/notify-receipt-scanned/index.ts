...
const notifyHard = z.object({
  ...
  // is a UUID. list_id is a client-generated list UUID, but validated only as a
  // non-empty string for consistency with the other Firebase-shaped IDs.
  user_id: z.string().min(1),
  family_group_id: z.string().min(1),
  list_id: z.string().min(1),
}).passthrough()

const notifyStrict = z.object({
  list_id: z.string().uuid(),
  store_name: z.string().max(100).optional(),
  list_name: z.string().max(200).optional(),
  user_name: z.string().max(100).optional(),
  item_count: z.number().int().min(0).max(10000).optional(),
  total_amount: z.number().min(0).max(1000000).nullable().optional(),
...
serve(async (req) => {
  ...
  try {
    ...
    const { idToken, family_group_id, user_id, user_name, store_name, list_id, item_count, total_amount, currency } = body

    if (!notifyHard.safeParse(body).success) {
      return new Response(
    ...
    // A caller may only announce their own purchase, to a group they belong to.
    ...
    if (!(await checkRateLimit(supabase, callerUid, 'notify-receipt-scanned', 5, 60))) {
      return rateLimited()
    }

    // Shadow-log stricter-rule violations (post-auth, sampled). Never rejects.
    await shadowLog(supabase, 'notify-receipt-scanned', strictIssues(notifyStrict, body))
    ...
    const displayName = (typeof user_name === 'string' && user_name) ? user_name : 'A family member'
    const count = typeof item_count === 'number' && Number.isFinite(item_count) && item_count > 0
      ? Math.floor(item_count)
      : null
    const countText = count != null ? `${count} item${count === 1 ? '' : 's'}` : 'some items'
    const totalText = typeof total_amount === 'number' && Number.isFinite(total_amount) && total_amount > 0
      ? `${typeof currency === 'string' ? currency : ''}${total_amount.toFixed(2)}`
      : null
    const bodyParts: string[] = []
    if (typeof store_name === 'string' && store_name) bodyParts.push(store_name)
    if (totalText) bodyParts.push(totalText)
    const notificationBody = bodyParts.length > 0 ? bodyParts.join(' · ') : 'Tap to see what they bought'
    ...
    const fcmPromises = tokens.map((tokenData) => {
      const message = {
        message: {
          ...
          notification: {
            title: `🧾 ${displayName} just bought ${countText}`,
