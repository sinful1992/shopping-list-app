...
serve(async (req) => {
  ...
  try {
    ...
    // Parse request body
    const { name, familyGroupId, userId } = await req.json()
    ...
    const listId = await firebasePush(`/familyGroups/${familyGroupId}/lists`, listData)

    // Update the list with its own ID
    await firebaseUpdate(`/familyGroups/${familyGroupId}/lists/${listId}`, { id: listId })

    // Increment usage counter
    await incrementUsageCounter(userId, 'listsCreated')
