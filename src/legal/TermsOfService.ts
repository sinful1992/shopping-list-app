/**
 * Terms of Service for Family Shopping List
 * Last updated: February 2026
 */

export const TERMS_OF_SERVICE_URL = 'https://sinful1992.github.io/familyshoppinglist-legal/terms.html';

export const TERMS_OF_SERVICE_CONTENT = `
# Terms of Service for Family Shopping List

**Last Updated: February 2026**

## Agreement to Terms

By downloading, installing, or using Family Shopping List ("the App"), you agree to be bound by these Terms of Service ("Terms"). If you do not agree to these Terms, do not use the App.

## Description of Service

Family Shopping List is a collaborative shopping list application that allows family members to:
- Create and share shopping lists in real-time
- Track shopping expenses and budgets
- Capture and store receipt images
- Receive notifications about urgent items
- Access premium features through subscription

## License Grant

We grant you a limited, non-exclusive, non-transferable, revocable license to use Family Shopping List on your personal device for personal, non-commercial purposes, subject to these Terms.

This license does not allow you to:
- Copy, modify, or distribute the App or any part of it
- Reverse engineer, decompile, or disassemble the App
- Rent, lease, lend, sell, or sublicense the App
- Use the App for any commercial purpose without our written consent

We may revoke this license at any time if you violate these Terms.

## Account Registration

### Requirements
- You must be at least 13 years old to use this App
- You must provide accurate and complete registration information
- You are responsible for maintaining the security of your account credentials
- You must notify us immediately of any unauthorized account access

### Family Groups
- Family groups allow multiple users to share shopping lists
- The group creator is responsible for managing group membership
- By joining a group, you consent to sharing your shopping activity with other members
- You may leave a group at any time through Settings

## User Conduct

You agree NOT to:
- Use the App for any illegal purpose
- Share your account credentials with non-family members
- Upload harmful, offensive, or inappropriate content
- Attempt to access other users' data outside your family group
- Reverse engineer or attempt to extract source code from the App
- Use automated systems to access the App
- Interfere with or disrupt the App's functionality

## Subscription and Payments

### Free Tier
- Basic features available at no cost
- Limited to core shopping list functionality

### Premium Subscriptions
- Monthly and yearly subscription options available
- Subscriptions are processed through Google Play
- Subscriptions auto-renew unless cancelled 24 hours before the renewal date
- Prices may vary by region and are subject to change

### Cancellation and Refunds
- Cancel anytime through Google Play subscription settings
- Refund requests are handled by Google Play per their refund policy
- No refunds for partial subscription periods

### Family Tier
- Premium features shared across family group members
- Family subscription pricing applies to the entire group

## Content and Data

### Your Content
- You retain ownership of content you create (lists, items, receipts)
- You grant us a license to store, process, and display your content to provide the service
- You are responsible for the accuracy of information you enter

### Receipt Images
- Receipt images are stored securely in the cloud
- Images are only accessible to your family group members
- We do not use receipt images for advertising or third-party purposes

### Data Deletion
- You may delete your account and all associated data at any time
- Deletion is permanent and cannot be undone
- Some anonymized analytics data may be retained

## Intellectual Property

- The App, including its design, features, and content, is owned by us
- "Family Shopping List" name and logo are our trademarks
- You may not copy, modify, or distribute any part of the App
- Third-party libraries are subject to their respective licenses

## Disclaimer of Warranties

THE APP IS PROVIDED "AS IS" WITHOUT WARRANTIES OF ANY KIND, EXPRESS OR IMPLIED, INCLUDING BUT NOT LIMITED TO:
- MERCHANTABILITY
- FITNESS FOR A PARTICULAR PURPOSE
- NON-INFRINGEMENT
- ACCURACY OF DATA
- UNINTERRUPTED SERVICE

We do not guarantee that:
- The App will be error-free or uninterrupted
- Data synchronization will always be immediate
- Receipt scanning will be 100% accurate

## Limitation of Liability

TO THE MAXIMUM EXTENT PERMITTED BY LAW:
- We are not liable for any indirect, incidental, special, or consequential damages
- Our total liability is limited to the amount you paid us in the past 12 months
- We are not responsible for data loss due to device issues, user error, or third-party services

## Indemnification

You agree to indemnify and hold harmless Family Shopping List, its officers, employees, and partners from any claims, damages, or expenses arising from:
- Your use of the App
- Your violation of these Terms
- Your violation of any third-party rights

## Third-Party Services

The App uses third-party services including:
- Firebase (Google) for authentication, database, and storage
- Supabase for push notifications
- RevenueCat for subscription management

Your use of these services is subject to their respective terms and privacy policies.

## Changes to Terms

We may modify these Terms at any time. Significant changes will be notified through:
- In-app notifications
- Email to registered users
- Updated date on this document

Continued use of the App after changes constitutes acceptance of the new Terms.

## Termination

We reserve the right to:
- Suspend or terminate accounts that violate these Terms
- Discontinue the App or any feature with reasonable notice
- Refuse service to anyone for any reason

Upon termination:
- Your right to use the App ceases immediately
- You may request deletion of your data

## Governing Law

These Terms are governed by and construed in accordance with the laws of the United Kingdom, without regard to conflict of law principles.

## Dispute Resolution

Any disputes shall be resolved through:
1. Good-faith negotiation between the parties
2. Mediation if negotiation fails
3. Binding arbitration or court proceedings as a last resort

## Severability

If any provision of these Terms is found unenforceable, the remaining provisions continue in full force and effect.

## Entire Agreement

These Terms, together with our Privacy Policy, constitute the entire agreement between you and Family Shopping List regarding the use of the App.

## Contact Us

For questions about these Terms:
- Email: legal@familyshoppinglist.app
- In-app: Settings > Contact Support
`;

export default TERMS_OF_SERVICE_CONTENT;
